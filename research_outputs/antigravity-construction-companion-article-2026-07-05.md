...
Start from the target: a **1,000 kg craft** with enough thrust to actually fly (not just barely hover), so aim for a thrust-to-weight of 1.5 — about **14.7 kilonewtons** of force. At the assumed performance (240 newtons from each 1-kilowatt superconducting cell), that's **62 cells drawing 62 kW of radio-frequency power.** So far, so reasonable.

One honest flag before we go a step further, because it's the load-bearing assumption of the whole piece: **that 240 N/kW is an *assumed actuator*, not a demonstrated device.** It is roughly **seventy million times** the thrust-per-watt of a photon rocket — so it is emphatically *not* a clever RF-engineering gain, it is the *assumed new physics* doing all the work. And it stacks two bets, not one: that the disputed, Eagleworks-scale cavity effect is real *at all* (later high-precision tests found none), **and** that it scales *linearly with cavity quality factor across six orders of magnitude* up to Q = 10¹⁰. Everything below asks a narrower question — *if* such a cell existed, could you build and fly the craft? — and answers only that. It never shows the cell can exist.
...
- A **fuelled turbo-generator** — a compact gas turbine — delivers **an hour on ~63 kg of fuel.** It is the *only* option that actually closes the budget at one ton. The propellantless dream, it turns out, still wants a tank of fuel — not to push against, but to keep the lights on. (Honest asterisk: a real microturbine is ~25–35 % efficient, so producing 126 kW of electricity means burning **~400 kW of fuel** and dumping the difference as exhaust heat — the ~63 kg is the *fuel*, and the generator hardware and its exhaust handling are their own first-order mass.)

## Module F — Getting rid of the heat

Almost all of those 126 kilowatts of *electrical* power end up as waste heat — from the amplifiers, the cryocooler compressors, the generator — and with no propulsive exhaust to carry it away you have to *radiate and blow* it off with liquid cold-plates and sized radiators. (The turbine adds its own, larger exhaust-heat stream on top — the ~400 kW of fuel it burns has to go somewhere too.) It's unglamorous, it's heavy, and it's a real line in the budget.

## Module G — Flight control

The craft is an unstable inverted pendulum in hover — it will tip over without a computer holding it upright thousands of times a second — so the flight computer is a safety-critical, triple-redundant heart. It reads an inertial measurement unit, altimeters, and (crucially) *per-sector thrust telemetry* so it knows the real force it's making, then solves a control-allocation problem: turn "pitch forward and hold altitude" into 62 individual RF amplitude commands. Because the array is wildly over-actuated — 62 thrusters for 6 degrees of freedom — it can **lose a cell and instantly reallocate to its neighbors**, exactly like a multirotor surviving a dead motor. At the simulated ~1.1-tonne mass the 62-cell array carries roughly **37 % raw thrust margin** over the craft's weight, so a single failed cell is a non-event — and, as the simulation below shows, even losing *eight at once* still leaves ~19 % authority for the allocator to lean on. (A design squeezed to only ~15 % margin would still shrug off single failures, but not many simultaneous ones — the multi-cell robustness genuinely depends on flying with real margin.)
...
Here's where this stops being a hand-wave. Everything above about the *control* of the craft — the fly-by-wire steering, the fault reallocation, the "hold hover to a few centimetres" — isn't a promise. It's a **runnable 6-degree-of-freedom simulation** of the whole thruster array and its flight computer, and you can download it and fly it yourself. (To be exact about the claim: the thrust *physics* is not proven — what the simulator flies is the **assumed-actuator model**. It proves the narrower, real thing: *if* a 62-cell array could produce the assumed forces, then the flight-control, state-estimation, control-allocation, slew-limited actuation, and fault-reallocation architecture holds together in software.)
...
1. **Cold is the wall.** Needing quality factors of ten billion forces 4 K superconductors, and cooling them dominates both the mass and the power. This single fact is why a true one-ton craft is marginal, and why the *first* real vehicle is almost certainly a **2–3 tonne machine** with modest payload — which means the thrust array has to grow *with* it. The 62-cell array we simulated hovers only up to about **1.5 tonnes**, so a 2-tonne craft needs on the order of **80 cells** and a 3-tonne one **120-plus**. (The 62-cell digital twin is deliberately the ~1.1-tonne class; a heavier prototype scales the array, not the physics.)
